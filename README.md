...
1.  *Clone the repository:*
    ...
    git clone <repository-url> cd api-proxy-server
...
3.  *Create a .env file:*
    ...
    PORT=3000 RATE_LIMIT=5 CACHE_DURATION=300 AUTH_TOKEN=your_auth_token WEATHERSTACK_API_KEY=your_weatherstack_api_key
...
*   *Method*: GET
*   *Headers*:
    *   Authorization: subscription_key your_auth_token

### Example Requests

#### Successful Request:

curl -H "Authorization: subscription_key your_auth_token" "http://localhost:3000/proxy?query=New Delhi"

#### Exceeding Rate Limit:


for i in {1..6}; do curl -H "Authorization: subscription_key your_auth_token" "http://localhost:3000/proxy?query=New Delhi"; done
...
Basic token-based authentication is implemented. The server expects an Authorization header with a subscription_key token. If the token is missing or invalid, a 401 status code is returned.
...
2024-07-29T15:30:00.000Z ::1 GET /proxy?query=New%20Delhi 200 50ms - within limit 2024-07-29T15:31:00.000Z ::1 GET /proxy?query=New%20Delhi 429 1ms - exceeded
